#Cara Gunakan visualizator:
#Klik 1 titik, merah dan biru utk titik awal dan tujuan. Lalu gambar obstacle (berwarn hitam)
#Jika sudah, tekan spasi untuk menjalankan algoritma
...
class Node:
    ...
    def update_neighbors(self, grid):
            self.neighbors = []
            if self.row < ROWCOL - 1 and  grid[self.row + 1][self.col].get_color()!=BARRIER_COLOR:
                self.neighbors.append(grid[self.row + 1][self.col])

            if self.row > 0 and grid[self.row - 1][self.col].get_color()!=BARRIER_COLOR:
                self.neighbors.append(grid[self.row - 1][self.col])
                
            if self.col < ROWCOL - 1 and grid[self.row][self.col+1].get_color()!=BARRIER_COLOR: 
                self.neighbors.append(grid[self.row][self.col+1])

            if self.col > 0 and grid[self.row][self.col - 1].get_color()!=BARRIER_COLOR: 
                self.neighbors.append(grid[self.row][self.col - 1])
